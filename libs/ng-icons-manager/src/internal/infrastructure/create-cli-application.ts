...
import { KeyboardPresetSelector, SetupCommand } from '../cli/setup-command';
...
export function createCliApplication(): CliApplication {
  ...
  const setup = new SetupCommand(fs, logger, new KeyboardPresetSelector(process.stdin, process.stdout));
