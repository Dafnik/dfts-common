...
export function getSymbolTotalCodewords(version: number): number {
  if (version < 0 || version > 40) throw new Error('"version" should be in range from 0 to 40');
...
let toSJISFunction: (kanji: string) => number;
export function setToSJISFunction(f: typeof toSJISFunction) {
