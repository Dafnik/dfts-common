...
export class TranslateStore {
  ...
  readonly translations$: Observable<translationFile> = this.selectedLanguage$.pipe(
    switchMap((language) => this.httpClient.get<translationFile>(`${this.assetsPath}${language}.json`).pipe(catchError(() => of({})))),
    shareReplay(1),
  );
  readonly autoGeneratedTranslations$: Observable<translationFile> = this.selectedLanguage$.pipe(
    switchMap((language) =>
      this.autoTranslatedLanguages.includes(language)
        ? this.httpClient.get<translationFile>(`${this.assetsPath}${language}_auto.json`).pipe(catchError(() => of({})))
        : of({}),
    ),
    shareReplay(1),
  );
